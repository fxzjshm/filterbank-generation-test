...
## source
source_name = "test"  # the name of the source observed
src_raj =   53432.937  # source location in hhmmss.s
src_dej = +220052.6  # source location in ddmmss.s

## file
nsamples = 600000  # target nsamples for a file, take care of memory
...
# TODO: presto uses this header but doesn't read this now
ifstream = "YYXX"  # "Y=IF present, X=IF not present (4 possible values)" --presto, see sigproc "IF stream". Not sure what these streams really are, but seems most use "YXXX" with nifs=1 and "YYXX" with nifs=2
